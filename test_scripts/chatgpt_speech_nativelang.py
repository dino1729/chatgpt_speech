...
'''
This script transcribes the native audio file to text, sends this native language text to GPT-3 for completion, and then converts the GPT-3 output in native language to audio response.
'''
...
def transcribe_audio(audio_file):
    ...
    native_text = format(result.text)

    return native_text, detectedSrcLang
...
while True:
    ...
    #Transcribe Telugu/Hindi audio to text using Azure Speech to Text
    native_text, detected_audio_language = transcribe_audio(audio_path)

    print("You said {} in {}".format(native_text, detected_audio_language))
    new_message = {"role": "user", "content": native_text}
    ...
    text_to_speech(assistant_reply, tts_output_path, detected_audio_language)
