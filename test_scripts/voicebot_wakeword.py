...
recording_duration = 8  # Duration for recording in seconds
...
try:
    ...
    print("Listening for the 'bumblebee' wake word...")
    ...
    while True:
        ...
        if porcupine.process(pcm_unpacked):
            ...
            if not recording:
                ...
                # Turn on the LED red
                update_led('ON', Color.RED, 0.75)
            else:
                # Ignore if already recording
                print("Already recording...")

        if recording and (time.time() - start_time >= recording_duration):
            recording = False
            print("Recording stopped. Processing audio...")
            sd.stop()
            sf.write(audio_path, recording_data, 44100, 'PCM_16')
            # Turn on the LED green breathing pattern
            update_led('BREATHE', Color.GREEN, 0.5)
    ...
    # Clean up GPIO
    GPIO.cleanup()
    update_led('OFF')  # Turn off the LED
